...
const EditProfile = () => {
    ...
    const editProfileHandler = async () => {
        ...
        try {
            setLoading(true);
            const res = await axios.post('https://insta-clone-fvrx.onrender.com/api/v1/user/profile/edit', formData,{
                headers:{
    ...
    return (
        <div className='flex md:max-w-2xl md:px-0 px-4 mx-auto md:pl-10 h-screen'>
            <section className='flex flex-col gap-6 w-full my-8'>
                ...
                <div className='flex items-center justify-between bg-gray-800 rounded-xl p-4'>
                    <div className='flex items-center gap-3'>
                        ...
                        <div>
                            <h1 className='font-bold text-sm text-white'>{user?.username}</h1>
                            <span className='text-gray-300 text-xs '>{user?.bio || 'Bio here...'}</span>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <Textarea value={input.bio} onChange={(e) => setInput({ ...input, bio: e.target.value })} name='bio' className="focus-visible:ring-transparent bg-gray-800 text-gray-300" />
                </div>
                <div>
